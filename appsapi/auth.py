...
# Trusted email domains — only these can register
...
def is_trusted_email(email: str) -> bool:
    """Check if an email belongs to a trusted domain."""
...
def create_user(db: Session, user_data: UserCreate) -> User:
    """Create a new user with email verification via Resend."""
    # Validate trusted email domain
    if not is_trusted_email(user_data.email):
        raise HTTPException(
            status_code=400,
            detail="Only Gmail, Outlook, Yahoo, ProtonMail, iCloud, and other trusted email providers are allowed. Please use a trusted email address."
        )
    ...
    email_sent = send_verification_email(user_data.email, user_data.name, verification_token)
    if email_sent:
        print(f"[Auth] Verification email sent to {user_data.email}")
    else:
        print(f"[Auth] WARNING: Verification email failed for {user_data.email}")
